...
export const findAll = async (req, res) => {
    try {
        ...
        res.status(200).json({ data: services, message: "Services retrieved successfully" });
    } catch (err) {
...
export const save = async (req, res) => {
    try {
        ...
        if (!Servicename || !Description || !Price) {
            return res.status(400).json({ message: "Servicename, Description, and Price are required" });
        }

        const services = await Services.create({ Servicename, Description, Price });

        res.status(201).json({ data: services, message: "Service created successfully" });
    } catch (err) {
...
export const findById = async (req, res) => {
    try {
        ...
        if (services) {
            res.status(200).json({ data: services, message: "Service retrieved successfully" });
        } else {
...
export const update = async (req, res) => {
    try {
        ...
        res.status(200).json({ data: services, message: "Service updated successfully" });
    } catch (err) {
